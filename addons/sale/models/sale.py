...
class SaleOrderLine(models.Model):
    ...
    def _get_price_actual(self):
        for line in self:
            line.price_actual =  line.price_unit * (1.0 - (line.discount or 0.0)/ 100.0) 

    @api.depends('product_id')
    def _get_price_recommended(self):
        for line in self:
            line.price_recommended =  line.product_id.price 

    @api.onchange('price_recommended', 'discount')
    def _onchange_discount_value(self):
        self.discount_value = self.price_recommended - self.price_recommended * (1 - (self.discount or 0.0)/ 100.0)
    ...
    price_actual = fields.Float(compute='_get_price_actual', string='Actual Price',  digits=dp.get_precision('Product Price'), default=0.0)

    price_subtotal = fields.Monetary(compute='_compute_amount', string='Subtotal', readonly=True, store=True)
    price_tax = fields.Monetary(compute='_compute_amount', string='Taxes', readonly=True, store=True)
    price_total = fields.Monetary(compute='_compute_amount', string='Total', readonly=True, store=True)

    price_unit = fields.Float( string='Price',  store=True,  readonly=True,  digits=dp.get_precision('Product Price'), default=0.0)
    
    price_recommended = fields.Float(compute='_get_price_recommended',string='Recommended Price',  store=True,  readonly=True,  digits=dp.get_precision('Product Price'), default=0.0)
    ...
    discount = fields.Float(string='Discount (%)', digits=dp.get_precision('Discount'), default=0.0, required=True)
    ...
    qty_available = fields.Float(
        compute='_get_available_qty', string='Stock On Hand', readonly=True,
        digits=dp.get_precision('Product Unit of Measure'))


    discount_value = fields.Float(
        string='Discounted value', readonly=True,
        digits=dp.get_precision('Product Unit of Measure'))
